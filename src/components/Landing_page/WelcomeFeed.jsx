...
function WelcomeFeed(props) {
	...
	const handlePostUpload = () => {
		...
		axios
			.post(`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/sources/upload`, file, {
				headers: {
	...
	const getDataUploads = (offsetUploads, firstTimeUploads, hasMoreUploads) => {
		...
		if (currentUser) {
			...
			currentUser.getIdToken().then((idtoken) =>
				axios
					.get(
						`${process.env.REACT_APP_API_URL || 'http://localhost:3001'}/sources/${
							search.length > 0 ? `?q=${search}&` : '?'
						}limit=${limit}&offset=${offsetUploads}&only_my=uploads`,
						{
							headers: {
								'id-token': idtoken,
							},
	...
	// Feed tab bar position handler
	useEffect(() => {
		switch (localStorage.getItem('feedTab')) {
			case 'my_works':
				setBarPosition('');
				break;
			case 'my_uploads':
				setBarPosition('after:translate-x-[100%] after:lg:translate-x-[150px]');
				break;
			case 'my_bookmarks':
				setBarPosition('after:translate-x-[200%] after:lg:translate-x-[300px]');
				break;
			case 'global':
				setBarPosition('after:translate-x-[300%] after:lg:translate-x-[450px]');
				break;
		}
	}, [localStorage.getItem('feedTab')]);
	...
	return (
		<div className="main-page-feed-section container xl:max-w-[1280px] mx-auto w-full drop-shadow-2xl ">
			...
			<div
				class={`text-sm font-light text-center text-gray-500  dark:text-zinc-300 dark:border-gray-700
			 			after:block after:lg:w-[150px] after:w-1/4 after:h-1 after:bg-green-400 after:transition-all
						after:duration-1000 after:ease-in-out after:rounded-t-lg ${barPosition}
					   `}
			>
			...
			<div
				className={`bg-zinc-50 dark:bg-darkMode dark:bg-mildDarkMode border-[1px] transition-all
								dark:border-none  rounded-[10px] sm:p-[40px] p-[10px] min-h-[40vh] duration-1000
			...
			>
				{feedTab === 'my_uploads' ? null : (
					...
					>
						...
						<Button type="submit" className="bg-zinc-700 text-[15px] ml-2 lg:ml-4 ">
							<svg
								aria-hidden="true"
								className="w-5 h-4"
								fill="none"
							...
							{isLoading
								? data.length > 0
									? data
											...
											.concat(
												[...Array(10)].map((item, index) => <SkeletonItem key={index + 500} />),
											)
				...
				{feedTab === 'my_uploads' && (
					<div className="">
						<div className="mt-5 mb-5  ">
							<Popover placement="bottom-start">
								<p className="text-l lg:text-xl text-zinc-700 dark:text-zinc-200 flex flex-col sm:flex-row font-sans">
									Now you can use Alphy on your audio files, privately.
									<PopoverHandler>
										<div>
											{' '}
											<p className="font-sans underline cursor-pointer mt-2 sm:mt-0 sm:ml-2">
												{' '}
												Learn more.
											</p>
								...
								<PopoverContent className="dark:bg-darkMode mt-2   border-slate-800 font-sans dark:text-zinc-200 text-zinc-600 max-w-[350px]">
									<div>
										<p className="mb-4">
											Premium users can transcribe, summarize, and question their own audio files
											in over 50 languages.{' '}
										</p>
										<div class="border-b border-gray-100 dark:border-zinc-700 mx-auto items-center flex mb-5 dark:opacity-40"></div>
										<ol>
											<li className="mb-2">
												1) Choose an audio file and upload to Alphy from below.
											</li>
											<li className="mb-2">
												2) Alphy will process your file the same way it does with online
												content, providing you the transcript, summary, key takeaways, and a
												chatbot.
											</li>
											<li className="mb-2">
												3) Only you will be able to access the end work. Your uploads will not
												be shown on Alphy's public database and will not be accessible by other
												parties.
											</li>
											<li className="mb-2">
												4) To preserve privacy, all audio files are deleted after the
						...
						{file === null ? (
							props.hasActiveSub === true ? (
								<div class="flex items-center justify-center w-full">
									<label
										{...getRootProps()}
										for="dropzone-file"
										class={`flex flex-col items-center justify-center w-full h-60 border-2 border-zinc-00 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-800 dark:bg-mildDarkMode hover:opacity-80 dark:border-gray-600 dark:hover:border-gray-700 dark:hover:bg-zinc-800 transition duration-200 ease-in `}
									>
										<div
											class={`flex flex-col items-center justify-center pt-5 pb-6 min-w-[200px] ${
												isDragActive ? '' : ''
											}`}
										>
											{!isDragActive ? (
												<div className="items-center justify-center flex flex-col">
													{!errorMessage ? (
														<div className="items-center justify-center flex flex-col">
															<svg
																aria-hidden="true"
																class="w-10 h-10 mb-3 text-zinc-600 dark:text-zinc-200"
																fill="none"
																stroke="currentColor"
																viewBox="0 0 24 24"
																xmlns="http://www.w3.org/2000/svg"
															...
															<p class="mb-2 text-sm text-zinc-700 dark:text-zinc-200">
																<span className="font-semibold">
																	Click to upload an audio file
																</span>{' '}
																or drag and drop.
															</p>
															<p class="text-xs text-zinc-600 dark:text-zinc-200">
																{' '}
												...
												<div className="items-center justify-center flex flex-col items-center">
													<svg
														className="w-10 h-10 mb-3 text-zinc-600 dark:text-zinc-200 items-center flex"
														aria-hidden="true"
														fill="none"
														stroke="currentColor"
														stroke-width="1.5"
								...
								<div>
									<div className="flex flex-col  col-span-2 mx-auto items-center">
										<p className="text-center text-zinc-500 dark:text-zinc-400 items-center margin-auto text-l mt-10 mb-5 w-full  col-span-2">
											You need to go{' '}
											<a className="text-green-400 underline" href="/plans">
												premium
											</a>{' '}
							...
							<div className="rounded-xl dark:border-darkMode bg-white dark:bg-darkMode p-10 drop-shadow-sm dark:drop-shadow-sm font-semibold ">
								<p
									className={`flex flex-row font-sans text-zinc-700 dark:text-zinc-200  ${
								...
								<div className="lg:flex lg:flex-row lg:grid lg:grid-cols-5">
									<p className="lg:col-span-2 flex  items-center font-sans text-zinc-700 dark:text-zinc-200 mt-8 lg:mt-0 ">
										{' '}
										{uploadTitle}
									</p>
									...
									<div className="lg:col-span-2 mt-2 ">
										<div className="lg:grid lg:grid-cols-3">
											<div className="lg:col-span-3 hidden lg:flex  lg:justify-center lg:mt-2 ">
												...
												{/*  */}{' '}
												{fileUploading === false && (
													<p className="text-sm  text-zinc-700 dark:text-zinc-200 italic font-sans w-full flex justify-center lg:mt-2">
					...
					{Math.floor(uploadDuration/60)}.{Math.floor(uploadDuration%60)} minutes
									...
									<div className="col-span-1 flex flex-col lg:flex-row lg:items-center lg:justify-center  lg:margin-auto">
										{fileUploading === 0 && (
											<p className="text-sm  text-zinc-700 dark:text-zinc-200 italic font-sans my-4 lg:hidden">
												Click continue to process the file...
											</p>
										)}
										<div
										...
										>
											<div
												class={`bg-green-400 h-3 rounded-full w-[${uploadProgress}%]`}
												style={{ width: uploadProgress + '%' }}
											></div>
										</div>
										...
										{fileUploading ? (
											...
											<div className="flex flex-row">
												...
												<Button
													...
													className="bg-green-400 lg:ml-10 normal-case max-w-[100px] my-4"
												>
													Continue
												</Button>
											</div>
