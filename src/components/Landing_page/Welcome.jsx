...
export default function Welcome({ hasActiveSub, credit }) {
		...
		if (
			...
			if (inputValue.includes('https://www.youtube.com')) {
				...
				if (hasActiveSub) {
					...
					toast.error(
						'Please switch to the premium plan to transcribe Twitter Spaces. See Account page for more detail.',
					);
					return;
				}
			}
	...
	return (
		<div
			className={`container xl:max-w-[1280px] px-4 mx-auto py-10 sm:py-18 lg:py-28 ${
				loading ? 'max-h-[90vh] overflow-x-hidden overflow-y-hidden' : ''
			}`}
		>
			{' '}
			...
			<div className="items-center container justify-center w-full mx-auto md:ml-10 lg:ml-40 xl:ml-60">
				<div className="flex  items-center mt-4 space-x-4 md:justify-center lg:mt-0  ">
					<div className="w-full flex flex-col">
						<p className=" text-l mb-2 text-zinc-700 dark:text-zinc-200 pb-2 ">
							<button
								id="popoverButton"
								data-popover-target="popoverDescription"
								data-popover-placement="left"
								data-popover-offset="20"
								type="button"
							>
								<svg
									className="w-5 h-5 ml-1 pt-1 dark:text-zinc-300 hover:dark:text-zinc-300 text-gray-500"
									aria-hidden="true"
									fill="currentColor"
									viewBox="0 0 20 20"
						...
						{currentUser && (
							<span className="text-sm pl-2 mb-2 text-gray-600 dark:text-zinc-300">
								<a href="/account" className="underline">
									{hasActiveSub ? 'Premium Plan' : 'Basic Plan'}
								</a>{' '}
						...
						<div
							...
							role="tooltip"
							className={`popover-description absolute z-10 invisible inline-block text-sm dark:text-zinc-300 text-gray-500 transition-opacity duration-300 bg-zinc-50 dark:bg-darkMode border border-slate-800 rounded-lg shadow-sm opacity-0 w-72`}
						>
				...
				<div className=" sm:grid sm:grid-cols-3 lg:grid-cols-4 mx-auto mt-5">
					<div class="sm:col-span-2 lg:col-span-2 relative w-full min-w-[200px] h-12">
						<input
							...
							className="peer w-full border-t-blue-gray-500 h-full bg-white dark:bg-mildDarkMode
										...
										dark:focus:border-t-transparent text-sm px-3 py-2.5 rounded-[7px] border-blue-gray-500
										dark:border-black dark:focus:border-r-green-400  dark:focus:border-l-green-400
										dark:focus:border-b-green-400 focus:border-green-400"
						/>
						<label
							class="text-zinc-400 flex w-full h-full select-none pointer-events-none absolute left-0
										font-normal peer-placeholder-shown:text-blue-gray-500 leading-tight peer-focus:leading-tight
										...
										text-blue-gray-400 peer-focus:text-green-400 before:border-blue-gray-200 dark:before:border-mildDarkMode
					...
					<div
						className={`sm:col-span-1 mt-5 sm:mt-0 flex ml-5 justify-center md:justify-self-start items-center ${
							currentUser ? '' : ''
						}`}
					>
						{currentUser ? (
